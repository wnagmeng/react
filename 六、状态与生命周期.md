...
它的作用：
1. 获取传入属性(props)
2. 初始化状态(state)，唯一可以也是必须直接设置state的地方
...
默认React的渲染是基于变化的，每次变化都会渲染。
通过shouldComponentUpdate可以有效的提升更新性能。
## 作用：
1. 返回值是`true`： 渲染
...
```
1. 帮助组件获取必要的DOM信息
2. 返回参数会作为componentDidUpdate的参数传递下去
